...
const submit = () => {
    router.get(route("auditoria.index", form.value));
};
...
const deleteAuditoria = () => {
    ...
    router.delete(
        route("auditoria.destroy", { auditorium: selectedAuditoria.value.id })
    );
...
<template>
    <AppLayout title="Dashboard">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                >
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                        ...
                        <table class="table-auto w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="p-3">Observacion</th>
                                    <th class="p-3">Descripcion</th>
                                    <th class="p-3">Porcentaje De Auditoria</th>
                                    <th class="p-3">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="au in auditorias.data" :key="au.id">
                                    <td class="p-3 border">{{ au.id }}</td>
                                    ...
                                    <td class="p-3 whitespace-normal">
                                        <div
                                            class="relative w-full bg-gray-200 rounded"
                                        >
                                            <div
                                                class="bg-blue-500 text-xs font-medium text-white text-center p-0.5 leading-none rounded"
                                                :style="{
                                                    width: `${au.porcentaje_auditoria}%`,
                                                }"
                                            >
                                                {{ au.porcentaje_auditoria }}%
                                            </div>
                                        </div>
                                    </td>
                        ...
                        <paginator class="mt-3" :paginator="auditorias" />
                    </div>
                </div>
            </div>
        </div>
        <dialog-modal :show="modalOpen">
            <template #title>
                <h1>Eliminar Auditoria</h1>
            </template>
            <template v-slot:content>
                <p v-if="selectedAuditoria">
                    ¿Seguro que quieres eliminar la Auditoria:
                    <strong>{{ selectedAuditoria.nombre }}?</strong>
                </p>
            </template>
            <template #footer>
                <PrimaryButton
                    class="mr-20 bg-red-500 hover:bg-red-800"
                    @click="deleteAuditoria()"
