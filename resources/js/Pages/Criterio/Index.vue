...
const selectedCriterio = ref(null);
...
const submit = () => {
    router.get(route("criterio.index", form.value));
};
...
const deleteCriterio = () => {
    ...
    router.delete(
        route("criterio.destroy", { criterio: selectedCriterio.value.id })
    );
...
<template>
    <AppLayout title="Dashboard">
                ...
                >
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                        ...
                        <table class="table-auto w-full">
                            ...
                            <tbody>
                                <tr v-for="cr in criterios.data" :key="cr.id">
                                    <td class="p-3 border">{{ cr.id }}</td>
                        ...
                        <paginator class="mt-3" :paginator="criterios" />
                    </div>
                </div>
            </div>
        </div>
        <dialog-modal :show="modalOpen">
            <template #title>
                <h1>Eliminar Criterio</h1>
            </template>
            <template v-slot:content>
                <p v-if="selectedCriterio">
                    ¿Seguro que quieres eliminar el Criterio:
                    <strong>{{ selectedCriterio.nombre }}?</strong>
                </p>
            </template>
            <template #footer>
                <PrimaryButton
                    class="mr-20 bg-red-500 hover:bg-red-800"
                    @click="deleteCriterio()"
