...
const selectedEscuela = ref(null);
...
const submit = () => {
    router.get(route("escuela.index", form.value));
};
...
const deleteEscuela = () => {
    ...
    router.delete(
        route("escuela.destroy", { escuela: selectedEscuela.value.id })
    );
...
<template>
    <AppLayout title="Dashboard">
        <template #header>
            ...
            >
                Programa de Estudios Acreditados
            </h2>
        </template>
                ...
                >
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                        ...
                        <table class="table-auto w-full">
                            ...
                            <tbody>
                                <tr v-for="e in escuelas.data" :key="e.id">
                                    <td class="p-3 border">{{ e.id }}</td>
                                    <td class="p-3 border">{{ e.nombre }}</td>
                                    <td
                        ...
                        <paginator class="mt-3" :paginator="escuelas" />
                    </div>
                </div>
            </div>
        </div>
        <dialog-modal :show="modalOpen">
            <template #title>
                <h1>Eliminar Escuela</h1>
            </template>
            <template v-slot:content>
                <p v-if="selectedEscuela">
                    ¿Seguro que quieres elimnar la Escuela:
                    <strong>{{ selectedEscuela.nombre }}?</strong>
                </p>
            </template>
            <template #footer>
                <PrimaryButton
