...
const selectedDinamico = ref(null);
...
const submit = () => {
    router.get(route("dinamico.index", form.value));
};

const deleteDinamico = () => {
    // if (!confirm("¿Seguro que quieres elimnar el usuario: " + data.email + "?"))
    //     return;

    router.delete(
        route("dinamico.destroy", { dinamico: selectedDinamico.value.id })
    );
...
<template>
    <AppLayout title="Dashboard">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                >
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                        ...
                        <table class="table-auto w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="p-3">Calificacion</th>
                                    <th class="p-3">Campo</th>
                                    <th class="p-3">Criterio</th> -->
                                    <th class="p-3">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="di in dinamicos.data" :key="di.id">
                                    <td class="p-3 border">{{ di.id }}</td>
                        ...
                        <paginator class="mt-3" :paginator="dinamicos" />
                    </div>
                </div>
            </div>
        </div>
        <dialog-modal :show="modalOpen">
            <template #title>
                <h1>Eliminar Estandar Dinamico</h1>
            </template>
            <template v-slot:content>
                <p v-if="selectedDinamico">
                    ¿Seguro que quieres eliminar el Estandar Dinamico:
                    <strong>{{ selectedDinamico.nombre }}?</strong>
                </p>
            </template>
            <template #footer>
                <PrimaryButton
                    class="mr-20 bg-red-500 hover:bg-red-800"
                    @click="deleteDinamico()"
